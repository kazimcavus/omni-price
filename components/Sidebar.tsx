...
export const Sidebar: React.FC<SidebarProps> = ({ selectedChannels, onToggleChannel, results, onToast }) => {
  ...
  return (
    <div className="bg-white h-fit rounded-xl shadow-sm border border-slate-200 p-5">
      <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider mb-5">Kanallar</h3>
      
      <div className="space-y-2.5 mb-8">
        {CHANNELS.map(ch => (
          <label key={ch.key} className="flex items-center space-x-3 cursor-pointer group p-2 rounded-lg hover:bg-slate-50 transition-all">
            <input
              type="checkbox"
              checked={selectedChannels.includes(ch.key)}
              onChange={() => onToggleChannel(ch.key)}
              className="h-5 w-5 text-brand-600 rounded border-slate-300 focus:ring-brand-500 focus:ring-2 transition-all cursor-pointer"
            />
            <span className="text-sm text-slate-700 group-hover:text-slate-900 transition-colors font-medium">{ch.label}</span>
          </label>
        ))}
      </div>
      ...
      <div className="pt-6 border-t border-slate-200">
        ...
        <button
          ...
          className="w-full flex items-center justify-center px-4 py-2.5 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-brand-600 hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500 transition-all"
        >
